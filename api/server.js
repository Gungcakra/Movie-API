...
app.get('/api/movie-list', async (req, res) => {
  try {
    const { data } = await axios.get('http://217.76.58.90/');
...
app.get('/api/movie-new', async (req, res) => {
    try {
      const url = 'http://217.76.58.90/?s=&search=advanced&post_type=&index=&orderby=&genre=&movieyear=2024&country=&quality=';
...
app.get('/api/movie-drama', async (req, res) => {
    try {
      const url = 'http://217.76.58.90/drama/';
      const { data } = await axios.get(url);
      const $ = load(data);
...
app.get('/api/movie-details/:movieId', async (req, res) => {
    ...
    const url = `http://217.76.58.90/${movieId}`;
...
app.get('/api/movie-search/:searchId', async (req, res) => {
  ...
  const url = `http://217.76.58.90/?s=${searchId}&post_type[]=post&post_type[]=tv`;
...
app.get('/api/movie-indo', async (req, res) => {
  const url = `http://217.76.58.90/?s=&search=advanced&post_type=movie&index=&orderby=&genre=&movieyear=&country=indonesia&quality=`;
...
app.get('/api/movie-genre/:genreId', async (req, res) => {  
  ...
  try {
    const url = `http://217.76.58.90/${genreId}/`;
    const { data } = await axios.get(url);
    const $ = load(data);
